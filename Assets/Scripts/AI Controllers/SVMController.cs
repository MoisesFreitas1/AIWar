...
public class SVMController : MonoBehaviour
{
    ...
    private SupportVectorMachine<Gaussian> decisionThrust, decisionSteer;
    ...
    void Start()
    {
        ...
        var teacherT = new SequentialMinimalOptimization<Gaussian>()
        {
            UseComplexityHeuristic = true,
            UseKernelEstimation = true // estimate the kernel from the data
        };

        var teacherS = new SequentialMinimalOptimization<Gaussian>()
        {
            UseComplexityHeuristic = true,
            UseKernelEstimation = true // estimate the kernel from the data
        };
    ...
    public SupportVectorMachine<Gaussian> GetDecisionThrust()
    {
    ...
    void CarInstantiate()
    {
        ...
        for (int i = 0; i < n_pods; i++)
        {
            ...
            car.GetComponent<SVMPlayerController>().SetCarParameters(POD_matrix[i]);
            car.GetComponent<SVMPlayerController>().SetTimeScale(timeScale);
        }
    }
    ...
    // Decision Tree
    public SupportVectorMachine<Gaussian> DecisionThrust(double[][] inputs, int[] outputs)
    {
        ...
        // Use the learning algorithm to induce the tree
        SupportVectorMachine<Gaussian> tree = teacher.Learn(inputs, outputs);

        return tree;
    }

    public SupportVectorMachine<Gaussian> DecisionSteer(double[][] inputs, int[] outputs)
    {
        var teacher = new SequentialMinimalOptimization<Gaussian>()
        {
            UseComplexityHeuristic = true,
